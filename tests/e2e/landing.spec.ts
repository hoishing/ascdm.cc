...
test.describe("ASCDM landing page", () => {
  test("renders the core landing content and email CTAs", async ({ page, isMobile }) => {
    ...
    await expect(page.getByRole("heading", { name: "免費工具" })).toHaveCount(0);
    await expect(page.getByRole("link", { name: "免費工具" })).toHaveCount(0);
  ...
  test("does not include removed free tools links", async ({ page }) => {
    await page.goto("/");

    await expect(page.locator("#free-tools")).toHaveCount(0);
    await expect(page.locator('a[href^="/webapps/"]')).toHaveCount(0);
